...
int printfiles(void)
{
  ...
  ifp2 = fopen("perfiles_fermion_inicial.dat","w");
...
void submain (long double rho_main)
{
    int i,j,k,m;
    int check;
    ...
    int jt;
        ...
        rho0 = rho_main;
            ...
            ifpar= fopen("tablamixtasemu10.dat","a");
...
int main (void){
  ...
  int condicion;
  long double rho_main;
  bool seguimiento;
  
  seguimiento = true;
  
  while (seguimiento == true){
    printf ("Quieres hacer una busqueda?...[0]\n");
    printf ("Quieres dejarla hasta ahí?...[1]\n");
    scanf("%d", &condicion);
    
    if (condicion == 0){
      printf("Escoge tu rho0 \n");
      scanf("%Lf", &rho_main);
      printf("has escogido %Lf \n", rho_main);
      submain(rho_main);
      printf("tu ultimo rho es %Lf \n", rho_main);
