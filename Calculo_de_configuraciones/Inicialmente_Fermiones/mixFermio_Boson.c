...
/****** enza con una estrella pura fermionica y se le añaden bosones****/
/***********************************************************************/
#include <stdio.h>
#include "nrutil.v2.c"
#include "nrutil.v2.h"
...
void derivs(long double x, long double y[], long double dydx[])
{
  ...
  if(y[6]>0.0) {
    //mu0 = pow(1.0/(4.0*PI),(1.0/gama-1.0))*pow(y[6]/kapa,1.0/gama) + y[6]/(gama-1.0);
    rhot= pow(1.0/(4.0*PI),(1.0/gama-1.0))*pow(y[6]/kapa,1.0/gama);
...
int printfiles(void)
{
  ...
  //  ifp1 = fopen("datos.dat","w");
    ifp2 = fopen("perfilesfermionboson.dat","w");
...
int main(void)
{
  int i,j,k,m;
  int check; 
  long double *v,dex,xf, Omega0,rhoi, rhof, Mass_F, Masa_ant;
  long double f1, f1min,f2,f2min,f3,f3min, Tol_mass, phi_1;
  
  //variables de cambio en el programa
  int conteorho, conteophi;
    
  int jt;
    //printf("Escriba el valor de la masa fija\n");
    printf("Escriba el valor de rho inicial\n");
    scanf("%LF", &rhoi);
    printf("Escriba el valor de rho final\n");
    scanf("%LE", &rhof);
    //printf("Escriba el valor de la toleracina Tol_mass\n");
    //scanf("%LF %LF %LF %LF",&Mass_F,&rhoi,&rhof,&Tol_mass);
    Mass_F = 0.6;
    Tol_mass = 5.0e-8;

    printf("La masa es %Le, rhoi %Le, rho0 %Le, precision %Le\n", Mass_F, rhoi, rhof, Tol_mass);
    
    phi0 = 0.0;
    //Ciclo para phi
  for(m=0; m<=100; m++)
    {
       par = fopen("mt/tablaparametrosF.dat","a");
       phi_1 = m*0.005;
       conteophi = 1;
       conteorho = 1;
    //phi0 = 0.0;
      ...
      {
        inicio:
        if (conteorho<41) rho0 = (rhof + rhoi)/2.0;
        else if (conteorho ==41){
          rhoi = rhoi-0.00001499;
          rhof = rhof+0.000011472;
          rho0 = (rhof+rhoi)/2.0;
        ...
        if(conteophi<4) phi0 = phi_1;
        else if (conteophi == 4){
          phi0 = phi0+0.00001;
          phi_1 = phi0;
          rhoi = rhoi-0.00002101;
          rhof = rhof+0.00005;
          rho0 = (rhof+rhoi)/2.0;
          printf("Se cambio la phi\n");
          printf("Se hace un nuevo ciclo con conteophi = %d\n", conteophi);
          conteophi = 1;
        }
        
        //printf("phi0=%14.8Le rho0=%14.8Le\n", phi0, rho0);
        ...
        if (matrizSingular == 1){
            ...
            phi0 = phi0 + 0.00001;
            phi_1 = phi0;
            printf("Se hizo un cambio en phi0\n");
            rhof = rhof + 0.0000351;
            printf("Se hizo un cambio en rhof \n");
            conteophi = 1;
            conteorho = 1;
            goto inicio;
          }
        ...
        printf("conteorho= %d m=%d phi0=%14.8Le rho0=%14.8Le Masa_tail=%14.8Le Dif_Masas=%14.8Le\n",conteorho, m, phi0, rho0, M_tail, Mass_F-M_tail);
        if(M_tail < Mass_F) rhoi=rho0;
        if(M_tail > Mass_F) rhof=rho0;
      ...
      printf("imprime datos en archivo");
      ...
      printf("Se encontro en paso %d para rho y %d para phi \n", conteorho, conteophi);
